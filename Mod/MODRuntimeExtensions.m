...
mod_propertyAttributes *mod_copyPropertyAttributes (objc_property_t property) {
    ...
    mod_propertyAttributes *attributes = calloc(1, sizeof(mod_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate mod_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        switch (flag) {
            ...
            case 'C':
                attributes->memoryManagementPolicy = mod_propertyMemoryManagementPolicyCopy;
                break;

            case '&':
                attributes->memoryManagementPolicy = mod_propertyMemoryManagementPolicyRetain;
                break;
